...
* Misskey Bot 自動投稿（イベント告知・リーダーボード新記録通知・イベント前日リマインド）
...
明日(JST)開催されるイベントについて、ユーザー連携済みの「参加」ステータスの参加者へ、ダイレクト投稿（`visibility: specified`）でメンション付きリマインドを送ります。ダイレクト投稿のため公開タイムラインには流れず、非公開イベントも対象です。

### CLIコマンド

| コマンド | 説明 |
|---------|------|
| `flask post-misskey-bot` | **推奨（Cron Job用）** — 全自動投稿を一括実行（イベント告知 + リーダーボード通知 + 前日リマインド） |
| `flask post-upcoming-events` | イベント告知のみ実行 |
| `flask post-leaderboard-records` | リーダーボード新記録通知のみ実行 |
| `flask post-event-reminders` | イベント前日リマインドのみ実行 |
