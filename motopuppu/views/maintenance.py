...
from sqlalchemy import or_, asc, desc, func # ★ func を追加
...
def add_maintenance():
    # (この関数は変更なし - 前回の修正を維持)
    user_motorcycles = Motorcycle.query.filter_by(user_id=g.user.id).order_by(Motorcycle.is_default.desc(), Motorcycle.name).all()
    if not user_motorcycles: flash('整備記録を追加するには、まず車両を登録してください。', 'warning'); return redirect(url_for('vehicle.add_vehicle'))
    form = MaintenanceForm(); form.motorcycle_id.choices = [(m.id, f"{m.name} ({m.maker or 'メーカー不明'})") for m in user_motorcycles]
    if request.method == 'GET':
        ...
        if preselected_motorcycle_id and any(m.id == preselected_motorcycle_id for m in user_motorcycles): form.motorcycle_id.data = preselected_motorcycle_id
    ...
    if form.validate_on_submit():
        ...
        new_entry = MaintenanceEntry(motorcycle_id=motorcycle.id, maintenance_date=form.maintenance_date.data, odometer_reading_at_maintenance=form.odometer_reading_at_maintenance.data, total_distance_at_maintenance=total_distance, description=form.description.data.strip(), location=form.location.data.strip() if form.location.data else None, category=form.category.data.strip() if form.category.data else None, parts_cost=form.parts_cost.data, labor_cost=form.labor_cost.data, notes=form.notes.data.strip() if form.notes.data else None)
        try:
            db.session.add(new_entry); _update_reminder_last_done(new_entry); db.session.commit()
            flash('整備記録を追加しました。', 'success'); return redirect(url_for('maintenance.maintenance_log'))
        except Exception as e:
            db.session.rollback(); flash(f'記録のデータベース保存中にエラーが発生しました。詳細は管理者にお問い合わせください。', 'error')
            current_app.logger.error(f"Error saving new maintenance entry: {e}", exc_info=True)
    elif request.method == 'POST': form.motorcycle_id.choices = [(m.id, f"{m.name} ({m.maker or 'メーカー不明'})") for m in user_motorcycles]
    return render_template('maintenance_form.html', form_action='add', form=form, today_iso=date.today().isoformat())
...
def edit_maintenance(entry_id):
    ...
    form = MaintenanceForm(obj=entry); form.motorcycle_id.choices = [(entry.motorcycle.id, f"{entry.motorcycle.name} ({entry.motorcycle.maker or 'メーカー不明'})")]; form.motorcycle_id.data = entry.motorcycle.id
    if form.validate_on_submit():
        motorcycle = entry.motorcycle
        offset_at_maintenance_date = motorcycle.calculate_cumulative_offset_from_logs(target_date=form.maintenance_date.data)
        total_distance = form.odometer_reading_at_maintenance.data + offset_at_maintenance_date
        entry.maintenance_date = form.maintenance_date.data; entry.odometer_reading_at_maintenance = form.odometer_reading_at_maintenance.data
        entry.total_distance_at_maintenance = total_distance; entry.description = form.description.data.strip()
        entry.location = form.location.data.strip() if form.location.data else None; entry.category = form.category.data.strip() if form.category.data else None
        entry.parts_cost = form.parts_cost.data; entry.labor_cost = form.labor_cost.data; entry.notes = form.notes.data.strip() if form.notes.data else None
        try:
            _update_reminder_last_done(entry); db.session.commit(); flash('整備記録を更新しました。', 'success')
    ...
    elif request.method == 'POST': form.motorcycle_id.choices = [(entry.motorcycle.id, f"{entry.motorcycle.name} ({entry.motorcycle.maker or 'メーカー不明'})")]; form.motorcycle_id.data = entry.motorcycle.id
...
def delete_maintenance(entry_id):
    ...
    entry = MaintenanceEntry.query.filter(MaintenanceEntry.id == entry_id).join(Motorcycle).filter(Motorcycle.user_id == g.user.id).first_or_404()
    try:
        db.session.delete(entry); db.session.commit(); flash('整備記録を削除しました。', 'success')
    except Exception as e:
...
def export_all_maintenance_logs_csv():
    ...
    all_maintenance_logs = MaintenanceEntry.query.filter(MaintenanceEntry.motorcycle_id.in_(user_motorcycle_ids))\
