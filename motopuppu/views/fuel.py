...
def fuel_log():
    ...
    # --- クエリにフィルター条件を追加 ---
    # request.args を辞書として保持 (テンプレートでフォームの値を維持するため)
    # to_dict(flat=False) にすると同じキーのパラメータをリストで取得できるが、今回はflat=True相当でOK
    ...
    try:
        ...
        if start_date_str:
            ...
            request_args_dict.pop('start_date', None) # なければ辞書からも削除
        ...
        if end_date_str:
            ...
            request_args_dict.pop('end_date', None) # なければ辞書からも削除
    ...
    if vehicle_id_str:
        try:
            ...
            if vehicle_id in user_motorcycle_ids:
                ...
                request_args_dict.pop('vehicle_id', None) # 無効ならクリア
        except ValueError:
             request_args_dict.pop('vehicle_id', None) # 数値でない場合もクリア
    else:
         request_args_dict.pop('vehicle_id', None) # 指定なしもクリア
    ...
    # --- 並び替えとページネーションの適用 ---
    # フィルター適用後のクエリに対して並び替えとページネーションを行う
    ...
    return render_template('fuel_log.html',
                           fuel_entries=fuel_entries,           # 表示する記録リスト
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            try:
                ...
                entry.price_per_liter = price_per_liter; entry.total_cost = total_cost
