...
def view_sheet(sheet_id):
    """整備情報シートの詳細を閲覧する"""
    sheet = MaintenanceSpecSheet.query.filter_by(id=sheet_id, user_id=g.user.id).first_or_404()
...
def edit_sheet(sheet_id):
    ...
    if request.method == 'GET':
        # DBから読み込んだJSONデータを文字列に変換してフォームの隠しフィールドに設定
...
def duplicate_sheet(sheet_id):
    """整備情報シートを複製する"""
    original_sheet = MaintenanceSpecSheet.query.filter_by(id=sheet_id, user_id=g.user.id).first_or_404()
    
    new_sheet = MaintenanceSpecSheet(
        user_id=original_sheet.user_id,
        motorcycle_id=original_sheet.motorcycle_id,
        sheet_name=f"{original_sheet.sheet_name} (コピー)",
        spec_data=original_sheet.spec_data  # JSONデータをそのままコピー
    )
