...
def get_advice(user, motorcycles):
    ...
    if user.nyanpuppu_simple_mode:
        ...
        static_tips = [
            # --- 機能紹介 ---
            ("給油記録を「満タン」で登録すると、自動で燃費が計算されるにゃん。", "blobcat.png"),
            ("整備記録にカテゴリを設定すると、リマインダーと連携できて便利にゃ。", "blobcat_thumbsup.png"),
            ("ダッシュボード右上のギアアイコンから、ウィジェットの配置を自由に変更できるにゃ。", "blobcataco.png"),
            ...
            ("統計ウィジェットの期間フィルタ、使ってるかにゃ？過去の記録を振り返るのに便利にゃん。", "blobcatuwu.png"),
            ("Misskey共有ボタンから、実績や活動記録を自慢できるにゃ！", "blobcat_yay.apng"),

            # --- バイクTIPS (メンテナンス) ---
            ("タイヤの空気圧はこまめにチェックするにゃ。安全運転の基本にゃん！", "blobcatuwu.png"),
            ...
            ("ヘルメットの耐用年数は3年と言われているにゃ。過ぎていたら交換を検討するのもいいかもにゃ！", "blobcat_oh.png"),
            ("バッテリーが弱ってないかにゃ？冬は特に上がりやすいから注意にゃ。", "blobcattea.png"),
            ("チェーンのたるみ、大丈夫かにゃ？指二本分くらいが目安にゃん。", "blobcataww.png"),
            ("ブレーキパッドの残量、溝を確認するにゃ。キーキー鳴ったら末期症状にゃん！", "blobcat.png"),
            ...
            ("タイヤの製造年月日、見たことあるかにゃ？4桁の数字で週と年がわかるにゃん。古すぎると危険にゃ！", "blobcat.png"),
