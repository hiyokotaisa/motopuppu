...
class BotNotificationLog(db.Model):
    """Botの自動投稿記録（重複防止用の汎用テーブル）"""
    __tablename__ = 'bot_notification_logs'
    id = db.Column(db.Integer, primary_key=True)
    notification_type = db.Column(db.String(100), unique=True, nullable=False, index=True,
                                  comment="通知タイプ (例: 'event_60days_123', 'leaderboard_record_456')")
    event_id = db.Column(db.Integer, nullable=True, comment="関連するイベントID（イベント通知の場合）")
